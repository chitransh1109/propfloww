...
const CardBody = styled.div`
  ...
  @media (max-width: 768px) { padding: 0.6rem; }
`
...
const CardPrice = styled.div`
  ...
  @media (max-width: 768px) { font-size: 0.88rem; margin-top: 0.25rem; }
`
const CardFooter = styled.div`
  display:flex; justify-content:space-between; align-items:center;
  margin-top: 1rem; padding-top:0.75rem; border-top:1px solid ${C.borderSubtle};
  @media (max-width: 768px) { margin-top: 0.4rem; padding-top: 0.4rem; }
`
...
`
const LeadPropertyHeader = styled.div`
...
`
const LeadDeleteBtn = styled.button`
...
function Dashboard() {
  ...
  // Buyers are silently switched to the 'owner' role in the background.
  const openAddPropertyModal = async () => {
    if (user?.role !== 'owner' && user?.role !== 'admin') {
      try {
        ...
        console.error('Failed to automatically switch role to owner:', err)
        alert('Failed to switch to owner role. Please try again.')
        return
      }
    }
    setShowModal(true)
    ...
    <Page>
      ...
      <Tabs>
        ...
        {(user?.role === 'owner' || user?.role === 'admin') && (
          <Tab $active={tab === 'leads'} onClick={() => setTab('leads')}>
            Buyer Enquiries ({leads.length})
          </Tab>
        )}
      </Tabs>
      ...
      {/* ── BUYER ENQUIRIES / LEADS TAB ── */}
      {tab === 'leads' && (
        <>
          <SectionHeading>Buyer <span>Enquiries & Leads</span> ({leads.length})</SectionHeading>
          {leads.length === 0 ? (
            <Empty>
              No active enquiries on your properties yet.
            </Empty>
          ) : (
            <LeadsContainer>
              {leads.map(lead => (
                <LeadCard key={lead._id}>
                  <LeadPropertyHeader onClick={() => navigate(`/properties/${lead.property?._id}`)}>
                    {lead.property?.title || 'Unknown Property'} ({lead.property?.city || 'N/A'})
                  </LeadPropertyHeader>
                  <LeadMetaInfo>
                    <InfoItem>
                  ...
                  <LeadFooter>
                    <LeadDate>
                      Received on {new Date(lead.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                    </LeadDate>
                    <LeadDeleteBtn onClick={() => deleteLead(lead._id)}>
                      Delete Enquiry
                    </LeadDeleteBtn>
                  </LeadFooter>
                </LeadCard>
