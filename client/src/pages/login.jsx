...
// ── OTP DIGIT BOX STYLES ─────────────────────────────────
const OtpWrap = styled.div`
  display: flex; gap: 0.7rem; justify-content: center; margin: 2rem 0;
`
const OtpBox = styled.input`
  width: 52px; height: 60px; text-align: center;
  font-family: 'Cormorant Garamond', serif; font-size: 2rem; font-weight: 600;
  color: ${C.white}; background: rgba(255,255,255,0.02);
  border: 1px solid ${p => p.$active ? C.gold : 'rgba(255,255,255,0.1)'};
  outline: none; transition: border-color 0.2s;
  caret-color: ${C.gold};
  &:focus { border-color: ${C.gold}; background: rgba(212,175,55,0.04); }
  @media (max-width: 480px) { width: 42px; height: 52px; font-size: 1.6rem; }
`
const ResendRow = styled.div`
  text-align: center; margin-top: 1.5rem; font-size: 0.82rem; color: ${C.muted};
`
const ResendBtn = styled.button`
  background: none; border: none; color: ${p => p.disabled ? C.muted : C.gold};
  font-size: 0.82rem; cursor: ${p => p.disabled ? 'default' : 'pointer'};
...
function Login() {
  ...
  const [bypassedCode, setBypassedCode] = useState('')
  ...
  useEffect(() => {
    ...
    if (params.get('expired') === 'true') {
      ...
      // Clear URL parameter so it doesn't persist on refresh
  ...
  const startOtpFlow = (email, code = '') => {
    setPendingEmail(email)
    setOtpDigits(['','','','','',''])
    setCountdown(60)
    setBypassedCode(code)
    setError('')
    setSuccess('')
    setMode('verify-otp')
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      if (data.otpBypassed && data.otpCode) {
        setBypassedCode(data.otpCode)
      } else {
        setBypassedCode('')
      }
      setOtpDigits(['','','','','',''])
    ...
    try {
      if (mode === 'forgot') {
        ...
        if (data.requiresVerification) {
          // Registration success → go to OTP verification step
          startOtpFlow(data.email || form.email, data.otpCode || '')
        } else {
          login(data)
          navigate(location.state?.from || '/properties')
        }
      ...
      // If login blocked for unverified user → go to OTP step
      if (errData?.requiresVerification) {
        startOtpFlow(errData.email || form.email, errData.otpCode || '')
        setError('Your email is not verified yet. Enter the OTP we sent you.')
      } else {
        setError(errData?.message || 'Something went wrong. Please check your credentials.')
      }
    }
    setLoading(false)
  }
    ...
    <>
      ...
      <Page>
        ...
        <Right>
          ...
          {mode === 'verify-otp' && (
            <FormWrap key="verify-otp">
              <MobileHeaderLogo onClick={() => navigate('/')}>Prop<span>Flow</span></MobileHeaderLogo>
              <FormEyebrow>Email Verification</FormEyebrow>
              <FormTitle>Check Your Inbox</FormTitle>
              <FormSub>
                We sent a 6-digit code to <strong style={{ color: C.white }}>{pendingEmail}</strong>. Enter it below.
              </FormSub>
              ...
              <OtpWrap>
                {otpDigits.map((digit, idx) => (
                  <OtpBox
                    ...
                    maxLength={1}
                    value={digit}
                    $active={document.activeElement === otpRefs[idx]?.current}
              ...
              <SubmitBtn onClick={handleVerifyOtp} disabled={loading}>
                {loading ? (
              ...
              <ResendRow>
                Didn't receive it?{' '}
                <ResendBtn onClick={handleResendOtp} disabled={countdown > 0 || loading}>
